...
describe("Daily Note Backlink Integration", () => {
  ...
  describe("Knowledge Worker Journey - Enhanced Frontmatter", () => {
    it("should generate enhanced frontmatter for new meetings", async () => {
      ...
      // Wait for sync to complete and files to be written
    ...
    it("should handle special characters in meeting titles", async () => {
      ...
      // Note: Special characters in filenames are sanitized, so the actual filename will be different
  ...
  describe("Edge Cases & Error Handling", () => {
    ...
    it("should maintain consistency on re-sync", async () => {
      ...
      // Assert: Content should be identical (except for sync timestamp)
      // Remove the sync timestamp line for comparison
      const removeSyncTimestamp = (content: string) => {
        return content.replace(/\*Synced from Granola on .+\*/, '*Synced from Granola*');
      };
      
      expect(removeSyncTimestamp(firstSyncContent)).toEqual(removeSyncTimestamp(secondSyncContent));
    ...
    it("should handle large number of meetings efficiently", async () => {
      ...
      for (let i = 1; i <= 50; i++) {
        manyMeetings.push({
          ...
          date: new Date(2024, 0, (i % 28) + 1, 9 + (i % 8), 0, 0),
      ...
      const sampleNote = await TestUtils.readMeetingNote("Performance Test Meeting 1 - 2024-01-02");
      expect(sampleNote).toContain('type: "meeting"');
      expect(sampleNote).toContain('daily-note: "[[2024-01-02]]"');
