...
describe.skip('FileLockManager - Simple Tests', () => {
  ...
  beforeEach(() => {
    ...
    (lockManager as any).maxRetries = 2;
    (lockManager as any).retryDelay = 10;
  });
  ...
  it('should prevent concurrent access to same file', async () => {
    ...
    // This should fail quickly due to our test configuration
  ...
  it('should handle file path normalization', async () => {
    ...
    // Should be blocked because it's the same file
    const lockId2 = await lockManager.acquireLock('/test/file.md', 'read');
    expect(lockId2).toBeNull();
